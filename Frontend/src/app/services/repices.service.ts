...
import { IRepice, IRepiceDto } from '../interfaces/i-repice';
...
export class RepicesService {
  ...
  getRepices(pag:number, size:number, sortField:string, sortDir:string, nombre:string, tipo:string, necesidades:string, dificultad:number, id_usuario:number):Observable<{count:number, result:IRepiceDto[]}> {
    ...
    return this.http.get<{count:number, result:IRepiceDto[]}>(this.repiceURL, {params}).pipe(
  ...
  followRepice = (idReceta:number):Observable<String> => this.http.put<{mensaje:string}>(`${this.repiceURL}/${idReceta}/seguimiento`, null).pipe(map(response=>response.mensaje))
